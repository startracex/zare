...
export default class Lexer {
  ...
  constructor(
    ...
    private isReturnBlock: boolean = false,
  ) {
    this.position = 0;
  ...
  readKeywordsOrText(tokens: Token[]) {
    ...
    while (
      this.currentCharacter &&
      !WHITESPACE_REGEX.test(this.currentCharacter) &&
      this.currentCharacter !== '<' &&
      this.currentCharacter !== ')' &&
      this.currentCharacter !== '('
    ) {
  ...
  readString(tokens: Token[]) {
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString('Unended string', {
        cause: 'String literal was not closed',
  ...
  readComponent(tokens: Token[]) {
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString('Expacting >', {
        cause: "'>' is missing",
        code: 'Template Format',
        columnNumber: this.column,
        lineNumber: this.line,
    ...
    if (word == '<')
      tokens.push({
        type: TOKEN_TYPES.LESSTHAN,
        value: word,
        line: this.line,
        column: this.column,
        filePath: this.filePath,
      });
    else if (DOCTYPE_REGEX.test(word))
      tokens.push({
    ...
    else if (CLOSINGTAG_REGEX.test(word))
      tokens.push({
    ...
    else if (SELFCLOSINGTAG_REGEX.test(word))
      tokens.push({
    ...
    else if (OPENINGTAG_REGEX.test(word))
      tokens.push({
  ...
  readFunctionCall(tokens: Token[]): boolean {
    ...
    while (this.currentCharacter && parentCount != 0) {
      if (parentCount == -1 && WHITESPACE_REGEX.test(this.currentCharacter)) {
        throw Template_Error.toString('Unexpected whitespace', {
          cause: 'Unexpected whitespace',
          code: 'Template Error',
          lineNumber: this.line,
          columnNumber: this.column,
          filePath: this.filePath,
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString("Unclosed '('", {
        cause: "Unclosed '('",
        code: 'Template Error',
        lineNumber: this.line,
        columnNumber: this.column,
  ...
  readParameter(tokens: Token[]) {
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString('Unended parameter expression', {
        cause: 'Unended parameter expression',
        code: 'Template Format',
        lineNumber: line,
        columnNumber: col,
        filePath: this.filePath,
  ...
  readCodeBlock(tokens: Token[]) {
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString("Unended block '}'", {
        cause: "Unended block '}'",
        code: 'Template Format',
        lineNumber: line,
        columnNumber: col,
  ...
  readIfStatement(tokens: Token[]) {
    ...
    if (!this.currentCharacter)
      throw Template_Error.toString("Unended parenthesis ')'", {
        cause: "Unended parenthesis ')'",
        code: 'Template Format',
        lineNumber: line,
        columnNumber: col,
        filePath: this.filePath,
  ...
  start(): Token[] {
    ...
    while (this.position < this.code.length) {
      ...
      if (
        this.currentCharacter == '@' &&
        this.code[this.position + 1] == 'i' &&
        this.code[this.position + 2] == 'f'
      ) {
      ...
      if (
        this.currentCharacter == '@' &&
        this.code[this.position + 1] == 'e' &&
        this.code[this.position + 2] == 'l' &&
        this.code[this.position + 3] == 's' &&
        this.code[this.position + 4] == 'e'
      ) {
      ...
      if (
        this.currentCharacter == '@' &&
        this.code[this.position + 1] == 'e' &&
        this.code[this.position + 2] == 'a' &&
        this.code[this.position + 3] == 'c' &&
        this.code[this.position + 4] == 'h'
      ) {
